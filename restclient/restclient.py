...
class Restclient:
    ...
    def _send_request(self, method, path, **kwargs):
        ...
        log = self.log.bind(event_id=str(uuid.uuid4()))  # инициализируем лог и присваиваем ему ключ с ID события, кот-й сразу преобразуем в строку, так как uuid отдает байтовую строку
...
# создаем CURL, чтобы наш запрос можно было юзать в постмане или слать разрабу
        ...
        curl = curlify.to_curl(response.request) #объект request у нас хранится в переменной response) Здесь мы и получим сurl
        print(curl)
